    ...
    $.fn.lightbox = function (options) {
        ...
        var plugin = {
            ...
            loadImage: function () {
                ...
                $("img, video", plugin.lightbox).remove();
                plugin.lightbox.fadeIn('fast').append('<span class="lightbox__loading"></span>');

                if ( /\.m4v$/.test( $(plugin.current).attr('href') ) ) {
                  img = $('<video controls autoplay name="media"><source src="' + $(plugin.current).attr('href') + '" type="video/mp4"></video>');
                  $(img).on( 'loadeddata', function () {
                      $('.lightbox__loading').remove();
                      plugin.lightbox.append(img);
                      plugin.image = $("img, video", plugin.lightbox).hide();
                      plugin.resizeImage();
                      plugin.setCaption();
                  });
                } else {
